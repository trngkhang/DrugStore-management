...
namespace DrugStore
{
    public partial class FAddThuoc : Form
    {
        ...
        private void FAddThuoc_Load(object sender, EventArgs e)
        {
            ...
            System.Data.DataTable dataTable = BUS.NhaCungCapBUS.Instance.getAllNCC();
            cbb_ncc.DataSource = dataTable;
            ...
            if (action)
            {
                ...
                cbb_ncc.SelectedValue = this.mancc;
            ...
            {
                DataRow[] rows = dataTable.Select($"MANCC = '{this.mancc}'");

                if (rows.Length > 0)
                {
                    int rowIndex = dataTable.Rows.IndexOf(rows[0]);
                    cbb_ncc.SelectedIndex = rowIndex;
                }
            }
        }
